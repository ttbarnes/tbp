import React from 'react';
import Helmet from 'react-helmet';
import LiveChat from 'react-livechat';
import PageContainer from '../PageContainer';
import H1 from '../../components/H1';
import H2 from '../../components/H2';
import { FadeIn } from '../../components/styledShared';

export class ContactPage extends React.Component { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
    this.state = {
      renderLiveChat: false
    };
    this.handleOnChatLoaded = this.handleOnChatLoaded.bind(this);
  }

  componentWillUnmount() {
    this.setState({
      renderLiveChat: false
    });
  }

  onClickLiveChat() {
    window.LC_API.open_chat_window();
  }

  handleOnChatLoaded() {
    const renderLiveChat = (window.LC_API &&
                            window.LC_API.open_chat_window &&
                            typeof window.LC_API.open_chat_window === 'function');
    if (renderLiveChat) {
      this.setState({
        renderLiveChat: true
      });
    } else {
      this.setState({
        renderLiveChat: false
      });
    }
  }

  render() {
    const { renderLiveChat } = this.state;

    return (
      <PageContainer smallWidth>

        <Helmet
          title="Contact"
          meta={[
            { name: 'description', content: 'Contact Tony' },
          ]}
        />

        <H1 hide>Contact</H1>
        <H2 h1Size>I{'\''}d love to talk, i{'\''}m very friendly!</H2>
        <FadeIn>
          <p><a href="mailto:tony@tonybarnes.me">tony@tonybarnes.me</a></p>
          <p><a href="http://twitter.com/ttbarnes" target="_blank" rel="noopener">@ttbarnes</a></p>
          {renderLiveChat && (
            <p>
              <button
                type="button"
                onClick={this.onClickLiveChat}
              >Chat now!
              </button>
            </p>
          )}
        </FadeIn>

        <LiveChat
          license={11437078}
          onChatLoaded={this.handleOnChatLoaded}
        />

      </PageContainer>
    );
  }
}

export default ContactPage;
